import tactic.core tactic.interactive
...
#eval simp_lemmas.mk_default simp_lemmas.empty >>= trace
...
meta def simp_all (s : acsimp.simp_lemmas) (to_unfold : list name) (cfg : simp_config := {}) (discharger : tactic unit := failed) : tactic unit :=
...
| loc.wildcard := do (all_hyps, s, u) ← mk_acsimp_set_core no_dflt attr_names hs tt,
                     if all_hyps then simp_all s u cfg discharger
...
by acsimp [and_not_self', false_and]
...
by acsimp [right_distrib, sub_mul, add_neg_self, add_sub_cancel,
