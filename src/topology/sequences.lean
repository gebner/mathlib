...
{ intro A, subst gbasis, rw mem_infi,
  { simp },
  { apply directed_of_mono, intros, apply principal_mono.mpr, apply gmon, assumption },
  { apply_instance }, },
classical, contrapose, simp only [not_forall, not_imp, not_exists, subset_def, @tendsto_def _ _ f, gbasis],
rintro ⟨B, hBl, hfBk⟩,
choose x h using hfBk,
use x, split,
{
  simp only [tendsto_at_top', gbasis], rintros A ⟨i, hgiA⟩,
  use i, intros j hij, apply hgiA, apply gmon, {assumption}, simp [h]
},
{
  simp only [tendsto_at_top', (∘), not_forall, not_exists],
  use B, use hBl, intro i, use i, use (le_refl _), apply (h i).right,
},
end
...
lemma tendsto_iff_seq_tendsto {α' β} (f : α' → β) (k : filter α') (l : filter β)
    (hcb : k.has_countable_basis) :
  tendsto f k l ↔ (∀ x : ℕ → α', tendsto x at_top k → tendsto (f ∘ x) at_top l) :=
suffices (∀ x : ℕ → α', tendsto x at_top k → tendsto (f ∘ x) at_top l) → tendsto f k l,
  from ⟨by intros; apply tendsto.comp; assumption, by assumption⟩,
